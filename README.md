...
| `press-feedback.js` | **Shared pressed state** — every enabled button, link, tab, radio and explicit press target scales to 0.96 in 120ms; floating controls tighten and darken their shadow while held. Reduced Motion keeps the state change but removes scale | [Interaction SSOT](https://www.figma.com/design/EHag6olZJxmlkf1hbAzSi7/Feed-Mobile-MVP?node-id=2868-26239) |
...
All enabled click targets use the same held state across the shell, MVP and
MVP onboarding: `scale 1 → .96` in 120ms, with release returning to rest over
the same duration. The individual `scale` property is animated separately from
structural transforms, so tab transitions, centered pills and sheet motion do
not move. Shadowed floating controls also compress their shadow: the new-feed
pill uses `10/20 → 3/7`, and the chart display button uses `4/15 → 1/5`, with a
denser opacity in both Light and Dark. Disabled controls do not react; Reduce
Motion removes scale and keeps the non-spatial visual state.
...
| any chart, wide or a tile in a row | the chart, fullscreen; its lower-left button switches candles / line without leaving the viewer, while the centered ticker and helper text share one details target | 3531:32240 |
