...
(async () => {
  ...
  try {
    for (const width of [320, 360, 393, 430]) {
      ...
      assert.equal(layout.height, 172);
      assert.equal(layout.padding, '16px');
      assert.equal(layout.gap, '8px');
      assert.deepEqual(layout.title, ['16px', '26px', '500', 'rgba(0, 0, 0, 0.9)']);
      assert.deepEqual(layout.description, ['12px', '20px', 'rgba(0, 0, 0, 0.5)']);
      assert.equal(layout.descriptionHeight, 40);
    ...
    assert.ok(middle.height > 0 && middle.height < 172, 'card collapses gradually');
