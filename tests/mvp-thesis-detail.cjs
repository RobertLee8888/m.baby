...
(async () => {
  ...
  try {
    ...
    assert.equal(Math.round(bounds.width), 393);
    assert.equal(Math.round((await media.locator('.thesis-media-item').first().boundingBox()).x), 48);
    ...
    assert.ok(Math.round((await media.locator('.thesis-media-item').first().boundingBox()).x) < 0);
    ...
    await page.goBack(); await settle();
    await detail().locator('.social-page-back').click(); await settle();
    assert.equal(await active().locator('.thesis-profile-list[aria-hidden="false"] .card').count(), 2);
    await cards.first().locator('.thesis-body').click(); await settle();
    assert.equal(await detail().locator('.thesis-view-updates').textContent(), 'View all 2 updates');
